...
function createWebReadableStream(url, size, innertube, videoInfo) {
      ...
      pull(controller) {
        ...
        return new Promise(async (resolve, reject) => {
          ...
          const fallback = [
            ...
            function() {
              if (!["IOS", "ANDROID"].includes(
                context.useClient
              ))
                return true;
              if (!["audio", "video"].includes(downloadOpts.type))
                return true;
              downloadOpts.quality = "best"; 
              downloadOpts.format = "any";
              downloadOpts.type = "audio";
              console.warn(
                `\x1B[33mTrying with ${downloadOpts} option\x1B[39m`
              );
              let fmtAudio;
              try {
                const fmtVideo = videoInfo.chooseFormat(downloadOpts);
                if (!fmtAudio || !fmtAudio.url) {
                  throw new Error("No suitable audio format found");
                }
                fetchUrl = fmtVideo.url;
              } catch (error) {
                console.log(`\x1B[31mFallback failed with audio-only: ${err.message}\x1B[39m`)
                return true;
              }
              return false;
            }
          ...
          for (let fallbackIndex = 0; fallbackIndex < fallback.length; fallbackIndex++) {
            try {
              ...
              const chunks = await innertube.actions.session.http.fetch_function(fetchUrl, {
                headers: {
...
async function stream(url, skipStream, options, tube) {
  ...
  const stream2 = skipStream ? null : createWebReadableStream(downloadedUrl, fmt.content_length, yt, info);
